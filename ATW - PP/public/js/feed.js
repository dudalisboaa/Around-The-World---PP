    ...
    if (currentUser) {
        ...
        if (headerUserAvatar) {
            if (currentUser.foto_perfil) {
                ...
                headerUserAvatar.src = '../assets/imagens/aviao.png';
...
async function handleCreatePost(event) {
    event.preventDefault(); // Evita recarregar a página no envio

    const form = event.target; // Formulário que disparou o evento
    const content = form.content.value.trim(); // Conteúdo digitado
    const photoInput = document.getElementById('photoInput'); // Input de imagem
    const submitButton = form.querySelector('button[type="submit"]'); // Botão "Postar"
    ...
    if (!currentUser) { // Verifica login
        showToast('Você precisa estar logado para postar', 'error');
        return;
    }

    setButtonLoading(submitButton, true); // Mostra "postando..."
    ...
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/posts/postar`, {
            method: 'POST',
            body: formData // O próprio FormData define o cabeçalho correto
        });
